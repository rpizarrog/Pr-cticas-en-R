---
title: "Caso 18. Distribución Normal"
...
![](images/formula%20densidad%20distribuciOn%20normal.jpg "Distribución Normal")

-   Para distribución normal estándar con media igual a 0 y desviación =
    1

-   Para distribución normal

$$
f(x)=\frac{1}{\sqrt{2\cdot\pi\cdot\sigma}}\cdot e^{-\frac{1}{2}\cdot(\frac{x-\mu}{\sigma})^2}
$$
...
prob = 1 / (desv* sqrt(2 * pi)) * (e ^(-(x-media)^2 / (2*desv^2)))
...
donde se cruzan el valor de con su probabilidad en distribuión normal.
...
y no la sumatoria de la función de densidad.
...
### Medias aritmeticas y desviaciones 
...
#### FEMENINO menor o igual a 50 KGS

¿Cuál es la probabilidad de encontrar a una persona femenino que pese
menor o igual de 50 kilogramos?

-   Graficar la función en donde $P(x \leq 50)$
-   Gráfica de densidad

```{r}
plotDist("norm", mean = media.peso.f, sd = desv.std.peso.f, groups = x <= 50, type = "h", xlab = "Peso Mujeres", ylab = "Densidad", sub=paste("Media = ",round(media.peso.f), " Desv. Std = ", round(desv.std.peso.f,4)) )
```
