---
title: "Caso 17. Distribución Poisson"
...
-   La variable de interés va desde el número de automóviles que llegan
    (llegadas) a un lavado de coches en una hora o
...
```{r}
tabla <- data.frame(x=0:20, f.prob.x = round(dpois(x = 0:20, lambda = media),4))
...
paste("La probabilidad de que el valor de x sea menor o igua a 10 es: ", tabla$f.acum[10])
```
...
```{r}
prob <- round(dpois(x = 1, lambda = media),4)
...
Regresando a la media $\mu = 10 \text { o }\lambda = 2$ , entonces la
esperanza es igual a: $2$

#### La varianza 

La varianza es igual a $2$
...
\
Se multiplica la cantidad la de accidentes por su probabilidad para
encontrar la media.
