...
importante. La probabilidad se emplea como herramienta; permite que se evalúe la confiabilidad de las conclusiones acerca de la población cuando tenga sólo información muestral [@mendenhall2010].

Por otra parte, la probabilidad indica el grado de certidumbre o certeza
de un suceso o fenómeno estudiado, en la investigación científica
existen muchos fenómenos en los cuales es necesario determinar la
probabilidad de que un evento ocurra o dejen de ocurrir, para lo cual el estudio de este campo, es necesario, además tiene aplicaciones muy importantes en investigación; dado que es base para la inferencia estadística que permite el estudio de muestras con el objetivo de inferir o extrapolar características de estas a una población.[@benitez_morales_probabilidad_nodate]
...
Existe un conjunto de axiomas que fueron formulados por el matemático ruso Kolmogórov. Por lo que se les denomina axiomas de
Kolmogórov.[@cevallos_enfoque_2018]
...
Ejemplo: Si de un total de personas existen un $60\%$ del género
femenino, ¿cuál es el complemento de ese subconjunto? y ¿su
probabilidad?. 
...
Si no hay elementos en común entre un conjunto A y B entonces se dice que la probabilidad de la intersección entre ambos es cero
$P(A\cap B) = \varnothing$

En dado caso de que si existan elementos en común entre un subconjunto
$A$ y $B$ $\therefore$ 
...
$$
P(conjunto) = casos / n \text{ o } \frac{n}{N}
$$

siendo $casos$ la frecuencia y $n$ el total de elementos de un universo.

Ejemplo: En el caso del ejemplo de las 100 personas y existen 40
hombres, ¿Cuál es la probabilidad de elegir a una persona y que ésta se del género masculino?: 
$$
n  = 100
$$
...
En el libro de [@mendenhall_introduccion_2010] se menciona que la
probabilidad de un evento $A$, dado que el evento $B$ ha ocurrido, se denomina probabilidad condicional de $A$, dado que $B$ ha ocurrido, denotada por 
...
Se carga la librería *knitr* previa instalación con
*install.packages("knitr")* que permite entre otras cosas, dar formato a las tablas de datos.

```{r message=FALSE, warning=FALSE}
# install.packages("knitr")
library(knitr) # Para formatear tablas en lugar de salida textual
```
...
Se identifican las frecuencias de personas que trabajan y no trabajan
hombre y mujeres en una ciudad pequeña X:
...
-   Construir un conjunto de datos con los totales usando funcion
    *apply()* que genera los márgenes totales por renglón y por columna.
...
Uno de estos individuos se seleccionará al azar para que realice viaje a
través del país para promover las ventajas de establecer industrias
nuevas en la ciudad (Durango, México). Nos interesaremos en los eventos
siguientes:

-   se elige a un hombre y el elegido tiene empleo o trabajo.

Entonces se elige a un hombre que trabaja (numerador de la fórmula de
probabilidad condicional):
...
y finalmente conforme la fórmula ¿cuál es la probabilidad de que se
elija a una persona que sea hombre dado que trabaja?:
$$P(A | B) = \frac{P(A \cap B)}{P(B)} $$
...
es $P(D) = 0.83$, la probabilidad de que *llegue* a tiempo es
...
b)  La probabilidad de que un avión haya salido a tiempo, dado que llegó
    a tiempo es:
...
-   ¿Qué porcentaje de aquellos que pasaron el primer examen también
    pasaron el segundo? Caso extraído de : [@hotmath_hotmath_nodate]

$$P(Ex1 \cap Ex2) = 0.30$$ $$P(Ex1) = 0.45$$ $$therefore$$
...
escolaridad, de una muestra aleatoria de 200 adultos. ejercicio extraído
de [@walpole2012].
